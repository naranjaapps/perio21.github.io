...
var Datos = /** @class */ (function () {
    ...
    Datos.prototype.generarAusenteHTML = function (filaTabla, tabindex) {
        ...
        ausente.setAttribute("tabindex", tabindex.toString());
        celda.appendChild(ausente);
        filaTabla.appendChild(celda);
    };
    Datos.prototype.generarMovilidadHTML = function (filaTabla, tabindex) {
        var celda = document.createElement('TD');
        celda.setAttribute("colspan", "3");
        var selectList = document.createElement("select");
        selectList.id = "movilidad" + this.numero.toString();
        selectList.setAttribute("tabindex", tabindex.toString());
    ...
    Datos.prototype.generarFaltaEQHTML = function (filaTabla, tabindex) {
        var celda = document.createElement('TD');
        celda.setAttribute("colspan", "3");
        var selectList = document.createElement("select");
    ...
    Datos.prototype.generarFurcaClaseHTML = function (filaTabla, tabindex) {
    ...
    Datos.prototype.generarFurcaMMHTML = function (filaTabla, tabindex) {
        for (var f = 0; f < this.furca_clase.length; f++) {
            ...
            inputMM.setAttribute("tabindex", tabindex.toString());
            //if (this.ausente) inputMM.setAttribute("hidden", this.ausente ? "true" : "false");
            celda.appendChild(inputMM);
            filaTabla.appendChild(celda);
        }
    };
    Datos.prototype.generarSupuracionHTML = function (filaTabla, tabindex) {
        var celda = document.createElement('TD');
        celda.setAttribute("colspan", "3");
        var selectList = document.createElement("select");
    ...
    Datos.prototype.generarPsHTML = function (filaTabla, tabindex) {
    ...
    Datos.prototype.generarUcaMGHTML = function (filaTabla, tabindex) {
        for (var f = 0; f < this.uca_mg.length; f++) {
            ...
            inputMG.setAttribute("tabindex", tabindex.toString());
...
var AreaGrafico = /** @class */ (function (_super) {
    ...
    AreaGrafico.prototype.IsLoaded = function () {
        return this.loaded;
    };
    AreaGrafico.prototype.redrawAusente = function (context, ausente) {
        for (var r = 0; r < this._lineas.length; r++) {
            var linea = this._lineas[r];
    ...
    AreaGrafico.prototype.redraw = function (context) {
        var _this = this;
        var d = new Image();
        if (this.posicion == Posicion.Superior)
            d.src = 'img/' + this.numero + '.webp';
        else
            d.src = 'img/' + this.numero + 'b.webp';
        d.onload = function () {
            context.drawImage(d, _this.x, _this.y, _this.width, _this.height);
            _this.loaded = true;
        };
    };
    return AreaGrafico;
...
var Diente = /** @class */ (function () {
    ...
    Diente.prototype.IsLoaded = function () {
        return this._areaGrafico.IsLoaded();
    };
    ...
    Diente.prototype.redrawAusente = function (context) {
        this._areaGrafico.redrawAusente(context, this.ausente);
    };
...
var Area = /** @class */ (function (_super) {
    ...
    Area.prototype.IsLoaded = function () {
        var loaded = true;
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            loaded = loaded && diente.IsLoaded();
        }
        return loaded;
    };
    ...
    Area.prototype.drawLines = function (context) {
        var posicion = this.dientes[0].posicion;
        var mitad_h = this.height / 2;
        var base_y = posicion == Posicion.Superior ? this.y + mitad_h + mitad_h / 2 : this.y + mitad_h / 2;
        var ancho = (this.width / this.dientes.length) / 3;
    ...
    Area.prototype.redraw = function (context) {
        ...
        var lines = setInterval(function () {
            if (_this.IsLoaded()) {
                clearInterval(lines);
                _this.drawLines(context);
                for (var r = 0; r < _this.dientes.length; r++) {
                    var diente = _this.dientes[r];
                    diente.redrawAusente(context);
                }
            }
        }, 200);
        context.beginPath();
    ...
    Area.prototype.generarFilaAusenteHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarAusenteHTML(fila, d + 1);
        }
    ...
    Area.prototype.generarFilaMovilidadHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarMovilidadHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
    Area.prototype.generarFilaFaltaEQHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarFaltaEQHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
    Area.prototype.generarFilaFurcaClaseHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarFurcaClaseHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
    Area.prototype.generarFilaFurcaMMHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarFurcaMMHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
    Area.prototype.generarFilaSupuracionHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarSupuracionHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
    ...
    Area.prototype.generarFilaPsHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarPsHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
    Area.prototype.generarFilaUCAHTML = function (tabla) {
        ...
        for (var d = 0; d < this.dientes.length; d++) {
            var diente = this.dientes[d];
            diente.datos.generarUcaMGHTML(fila, d + 1);
        }
        tabla.appendChild(fila);
    };
...
var PeriodontogramaApp = /** @class */ (function () {
    ...
    PeriodontogramaApp.prototype.IsLoaded = function () {
        var loaded = true;
        for (var r = 0; r < this.areas.length; r++) {
            var area = this.areas[r];
            loaded = loaded && area.IsLoaded();
        }
        return loaded;
    };
    ...
    PeriodontogramaApp.prototype.click = function (mouseX, mouseY) {
        var redraw = false;
        for (var r = 0; r < this.areas.length; r++) {
            var area = this.areas[r];
    ...
    PeriodontogramaApp.prototype.setAusenteContraparte = function (area_origen_index) {
        var area_destino_index = 0;
        if (area_origen_index in [0, 1, 2])
            area_destino_index = area_origen_index + 3;
        else if (area_origen_index in [3, 4, 5])
            area_destino_index = area_origen_index - 3;
        else if (area_origen_index in [6, 7, 8])
            area_destino_index = area_origen_index + 3;
        else if (area_origen_index in [9, 10, 11])
            area_destino_index = area_origen_index - 3;
        var area_origen = this.areas[area_origen_index];
        var area_destino = this.areas[area_destino_index];
        for (var d = 0; d < area_destino.dientes.length; d++) {
            area_destino.dientes[d].ausente = area_origen.dientes[d].ausente;
            area_destino.dientes[d].datos.ausente = area_origen.dientes[d].datos.ausente;
        }
    };
